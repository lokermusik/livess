...
(function ($) {
	...
	$.fn.myCast = function (options) {
        var settings = $.extend({
			...
			enable_cors: false,
			cors: "https://cors.tundracast.com",
			artwork: true,
            logo: "https://widget.tundracast.com/images/logo.jpg",
		...
		thisObj.each(function () {
            ...
            if(settings.version == 1) {
                audio.src = 'http://' + settings.serverID + '.tundracast.com' +':' + settings.serverPort + "/;type=mp3";
                settings.src = audio.src;				
                var dataURL = settings.cors + "/" + 'http://' + settings.serverID + '.tundracast.com' +':' + settings.serverPort + "/7.html";
                var hisURL = settings.cors + "/" + 'http://' + settings.serverID + '.tundracast.com' +':' + settings.serverPort + "/played.html";
                getSD(dataURL, hisURL);
            }

            else if(settings.version == 2) {
				audio.src = 'http://' + settings.serverID + '.tundracast.com' +':' + settings.serverPort + settings.streampath;
				settings.src = audio.src;				
                if(settings.enable_cors == true) {
					var dataURL = settings.cors + "/" + 'http://' + settings.serverID + '.tundracast.com' +':' + settings.serverPort + "/stats?sid="+ settings.stream_id +"&json=1&callback=?";
					var hisURL = settings.cors + "/" + 'http://' + settings.serverID + '.tundracast.com' +':' + settings.serverPort + "/played?sid="+ settings.stream_id +"&type=json&callback=?";
				}
				else {
					var dataURL = 'http://' + settings.serverID + '.tundracast.com' +':' + settings.serverPort + "/stats?sid="+ settings.stream_id +"&json=1&callback=?";
					var hisURL = 'http://' + settings.serverID + '.tundracast.com' +':' + settings.serverPort + "/played?sid="+ settings.stream_id +"&type=json&callback=?";
				}
                getSD(dataURL, hisURL);				
            }

            else if(settings.version == "tundracast") {
                audio.src = 'http://' + settings.serverID + '.tundracast.com' +':' + settings.serverPort + "/" + settings.mount_point;
                settings.src = audio.src;
                var dataURL = `${settings.cors}/http://${settings.serverID}.tundracast.com:${settings.serverPort}/status-json.xsl`;
                getIC(dataURL);				
            }
		...
		//Icecast
		function getIC(url) {		                
            if(settings.version == "tundracast") {
			    function foo() {
                    $.ajax ({
                    dataType: 'json',
                    url: url,
